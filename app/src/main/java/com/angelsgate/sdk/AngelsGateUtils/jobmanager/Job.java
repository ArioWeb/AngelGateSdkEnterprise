...
public abstract class Job extends Worker implements Serializable {
    ...
    Result doWorkInternal() {
        ...
        System.out.println("testJob" + "doWorkInternal ");
        ...
        try {
            if (withinRetryLimits(data)) {
                ...
                if (needsForegroundService(data)) {
                    Log.i(TAG, "test Running a foreground service with description '" + getDescription() + "' to aid in job execution." + logSuffix());
...
//                }
                ...
                warn("test Failing after hitting the retry limit." + logSuffix());
                return cancel();
            }
        } catch (IOException e) {
            if (onShouldRetry(e)) {
                ...
                log("test Retrying after a retryable exception." + logSuffix(), e);
                return retry();
            }
            warn("test Failing due to an exception." + logSuffix(), e);
            return cancel();
        } catch (Exception e) {
            if (onShouldRetry(e)) {
                log("test Retrying after a retryable exception." + logSuffix(), e);
            ...
            if (foregroundRunning) {
                Log.i(TAG, "test Stopping the foreground service." + logSuffix());
...
//    Data.Builder serialize(@NonNull Data.Builder dataBuilder);
     ...
     * @throws Exception
     */
    public abstract Result onRun() throws Exception;
